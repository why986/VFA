...
class BFM09ReconModel(BaseReconModel):
    ...
    def render_mesh(self, vs, face_color, angles=None):
        if angles is not None:
            rotation = self.compute_rotation_matrix(angles)
            vs = self.rigid_transform(vs, rotation, torch.tensor([0.6, 0.3, 0.]).to(self.device))
        mesh = Meshes(vs, self.tri.repeat(1, 1, 1), TexturesVertex(face_color))
    ...
    def forward(self, coeffs, render=True):
        ...
        if render:
            ...
            rendered_img = self.renderer(mesh)
            depth_img = self.renderer.rasterizer(mesh).zbuf
            pix_to_face = self.renderer.rasterizer(mesh).pix_to_face
            pix_dists = self.renderer.rasterizer(mesh).dists
            bary_coords = self.renderer.rasterizer(mesh).bary_coords
